...
def get_html_from_attributes(
    ...
    if attributes is not None:
        ...
        if attributes.get('run'):
            ...
            html_str = form_hyperlink_html_str(
                display_text='Fallout test',
                url=fallout.get_test_url(test_name)
            )
        ...
        if attributes.get('commit'):
            html_str += form_hyperlink_html_str(
                display_text='Git commit',
                url=f"{repo_url}/commit/{attributes.get('commit')}"
            )
        elif attributes.get('branch'):
            html_str += form_hyperlink_html_str(
                display_text='Git branch',
                url=f"{repo_url}/tree/{attributes.get('branch')}"
            )
    else:
        html_str = form_hyperlink_html_str(
            display_text='Fallout test',
